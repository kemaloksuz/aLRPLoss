...
def plot_curve(log_dicts, args):
    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for j, metric in enumerate(metrics):
            ...
            if metric not in log_dict[epochs[0]] and metric!='total':
            ...
            if 'mAP' in metric:
                ...
                #if legend[i*num_metrics+j][5:7] == 'wo':
                #ys = moving_average(np.asarray(ys).reshape(-1), 50)
                #ys = moving_average(np.asarray(ys).reshape(-1), 100)
                #xs = np.arange(0, len(ys), 1)
                #pdb.set_trace()
                plt.xlabel('iter')
                #if legend[i*num_metrics+j] == 'AP_reg' or legend[i*num_metrics+j] == 'AP_total':
                #    continue
...
def load_json_logs(json_logs, keys):
    ...
    for json_log, log_dict in zip(json_logs, log_dicts):
        with open(json_log, 'r') as log_file:
            for l in log_file:
                ...
                if 'total' in keys:
                    #log['regression_loss_current'] *= 10
...
def main():
    ...
    log_dicts = load_json_logs(json_logs, args.keys)
    #pdb.set_trace()
