...
class BBoxTransform(nn.Module):
    ...
    def forward(self, boxes, deltas):
        ...
        pred_boxes_x1 = pred_ctr_x - 0.5 * torch.clamp(pred_w-1.0,min=0)
        pred_boxes_y1 = pred_ctr_y - 0.5 * torch.clamp(pred_h-1.0,min=0)
        pred_boxes_x2 = pred_ctr_x + 0.5 * torch.clamp(pred_w-1.0,min=0)
        pred_boxes_y2 = pred_ctr_y + 0.5 * torch.clamp(pred_h-1.0,min=0)
