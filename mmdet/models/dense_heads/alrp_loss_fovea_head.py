...
class aLRPLossFoveaHead(BaseDenseHead):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 test_cfg=None):
        super(aLRPLossFoveaHead, self).__init__()
    ...
    def get_bbox_targets(self, raw_bboxes, featmap_sizes, points, num_imgs):
        bbox_targets = []
        start_idx = 0
        raw_bboxes = raw_bboxes.exp()
        for i in range(num_imgs):
            for featmap_size, stride, base_len, (y, x) \
                in zip(featmap_sizes, self.strides, self.base_edge_list, points):
                end_idx = start_idx + featmap_size[0]*featmap_size[1]
                x1 = (stride * x.reshape(-1) - base_len * raw_bboxes[start_idx:end_idx, 0])
                y1 = (stride * y.reshape(-1) - base_len * raw_bboxes[start_idx:end_idx, 1])
                x2 = (stride * x.reshape(-1) + base_len * raw_bboxes[start_idx:end_idx, 2])
                y2 = (stride * y.reshape(-1) + base_len * raw_bboxes[start_idx:end_idx, 3])
                bboxes = torch.stack([x1, y1, x2, y2], -1)
                bbox_targets.append(bboxes)
                start_idx = end_idx
        bbox_targets_ = torch.cat(bbox_targets)
        return bbox_targets_
    ...
    def loss(self,
        ...
        flatten_bbox_targets= self.get_bbox_targets(flatten_bbox_targets, featmap_sizes, points, num_imgs)
        flatten_bbox_preds= self.get_bbox_targets(flatten_bbox_preds, featmap_sizes, points, num_imgs)
        ...
        if num_pos > 0:
            ...
            loss_cls, rank, order = self.aLRP_Loss.apply(flat_preds, flat_labels, loss_bbox)
            ...
            loss_bbox = ((torch.cumsum(ordered_losses_bbox,dim=0)/rank[order.detach()].detach().flip(dims=[0])).mean())
