...
# 2. Input 
input_image = dict(image_size = 800, #for input S, training image size is SxS
	               norm_mean = np.array([[[102.9801, 115.9465, 122.7717]]]), #Mean to normalize the data
	               norm_std = np.array([[[58.395, 57.120, 57.375]]]), #Standard Deviation to normalize the data
...
optimization = dict(image_per_gpu= 4, # Number of images per GPU
...
test = dict(epoch = [99], # Epochs to test
	        image_size = [800], # List of scales to test. Provide a list for multiscale test.
